export default async function handler(req, res) {
  ...
  try {
    const r = await fetch(`${supabaseUrl}/rest/v1/reservations?date=eq.${targetDate}&statut=neq.annule&select=*`, {
    ...
    return res.status(200).json({ success: true, date: targetDate, checked: reservations.length, sent });
  } catch (error) {
