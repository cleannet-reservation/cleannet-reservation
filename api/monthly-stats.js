export default async function handler(req, res) {
  ...
  try {
    ...
    if (brevoKey && ownerPhone) {
      await fetch("https://api.brevo.com/v3/transactionalSMS/sms", { method: "POST", headers: { "Content-Type": "application/json", "api-key": brevoKey }, body: JSON.stringify({ sender: "CleanNet", recipient: `+${ownerPhone.replace(/\D/g,"")}`, content: smsText, type: "transactional" }) });
    }
