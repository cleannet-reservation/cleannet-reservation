export default async function handler(req, res) {
  ...
  try {
    await fetch("https://api.brevo.com/v3/smtp/email", { method: "POST", headers: { "Content-Type": "application/json", "api-key": brevoKey }, body: JSON.stringify({ sender: { name: "CleanNet Avis", email: senderEmail }, to: [{ email: ownerEmail }], subject: `⚠️ Avis négatif ${stars}`, htmlContent: html }) });
