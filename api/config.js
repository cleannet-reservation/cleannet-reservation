const getBase = () => { const url = (process.env.SUPABASE_URL || "").replace(/\/rest\/v1\/?$/, ""); return `${url}/rest/v1/config`; };
...
export default async function handler(req, res) {
  ...
  if (req.method === "GET") {
    ...
    if (Array.isArray(data) && data.length > 0 && data[0].data && Object.keys(data[0].data).length > 0) return res.status(200).json(data[0].data);
  ...
  if (req.method === "POST") {
    ...
    const r = await fetch(`${base}?id=eq.main`, { method: "PATCH", headers: { ...headers(), "Prefer": "return=representation" }, body: JSON.stringify({ data: configData, updated_at: new Date().toISOString() }) });
