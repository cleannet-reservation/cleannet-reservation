export default async function handler(req, res) {
  ...
  const ownerHtml = `<div style="font-family:Inter,sans-serif;padding:20px;"><h2>${avecAcompte?"🔔 Nouvelle réservation":"📋 Nouvelle demande"}</h2><p><strong>${prenom} ${nom}</strong><br>📞 ${telephone}<br>📧 ${email}<br>📍 ${adresse}</p><p>🧹 ${service}${option?" — "+option:""}<br>📅 ${date} · ${creneau}<br>💶 Total : ${total}</p></div>`;
  const clientHtml = `<div style="font-family:Inter,sans-serif;padding:20px;"><h2>${avecAcompte?"✅ Réservation confirmée !":"📬 Demande bien reçue !"}</h2><p>Bonjour <strong>${prenom}</strong>,</p><p>🧹 ${service}${option?" — "+option:""}<br>📅 ${date} · ${creneau}<br>📍 ${adresse}<br>💶 Total : ${total}</p><p>CleanNet Multi-Service 06</p></div>`;
  try {
    if (!smsOnly) {
