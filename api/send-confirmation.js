export default async function handler(req, res) {
  ...
  const { prenom, nom, email, telephone, adresse, service, option, date, creneau, total, acompte, message, statut, smsOnly } = req.body;
  ...
  const ownerHtml = `<div style="font-family:Inter,sans-serif;padding:20px;"><h2>${avecAcompte ? "🔔 Nouvelle réservation" : "📋 Nouvelle demande"}</h2><p><strong>${prenom} ${nom}</strong><br>📞 ${telephone}<br>📧 ${email}<br>📍 ${adresse}</p><p>🧹 ${service} — ${option}<br>📅 ${date} · ${creneau}<br>💶 Total : ${total}${avecAcompte ? `<br>💳 Acompte : ${acompte}` : ""}</p></div>`;
    ...
    if (telephone) {
      ...
      try {
        const smsRes = await fetch("https://api.brevo.com/v3/transactionalSMS/sms", {
          method: "POST",
          headers: { "Content-Type": "application/json", "api-key": process.env.BREVO_API_KEY },
          body: JSON.stringify({ sender: "CleanNet", recipient: tel, content: smsText, type: "transactional", tag: "reservation" }),
        });
